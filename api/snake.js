/*
  API de la tabla de posiciones global de Neon Snake Survival — usado
  por js/snake.js en play/snake.html. Mismo patrón que api/dash.js
  (Redis sorted set), con sus propias claves para no mezclar los
  puntajes de los dos juegos.

  GET  /api/snake?visitorId=v123        -> { top: [{name, score}, ...], you: {rank, score} | null }
  POST /api/snake { name, score, visitorId } -> guarda el puntaje si es un
       nuevo mejor para ese visitorId, devuelve la misma forma que el GET.
*/

const { createClient } = require('redis');

const BOARD_KEY = 'snake:board';
const NAMES_KEY = 'snake:names';
const TOP_N = 10;
