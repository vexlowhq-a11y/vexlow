/*
  API de la tabla de posiciones global de Vex Dash — usado por js/dash.js
  en play/dash.html. Guarda el mejor puntaje de cada visitante en un
  sorted set de Redis (mismo patrón que api/react.js y api/trivia.js).

  GET  /api/dash?visitorId=v123        -> { top: [{name, score}, ...], you: {rank, score} | null }
  POST /api/dash { name, score, visitorId } -> guarda el puntaje si es un
       nuevo mejor para ese visitorId, devuelve la misma forma que el GET.
*/

const { createClient } = require('redis');

const BOARD_KEY = 'dash:board';
